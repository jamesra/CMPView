...
%          3: like 0, using mahal distance measurement to adjust membership
%          (JamesAn, U of Utah)
% distance = Distance measure to use, mahal or sqeuclidian
...
while nchanged > 0
    ...
    if (method == 0 & ~skipestep)
        ...
        for i = 1:n
            dataPoints = data(i,:);
            parfor j = 1:k
                distmat(i,j) = calcdist(dataPoints,centers(j,:));
            end
        end
    ...
    elseif (method == 1 | (method == 3) | (method == 2 & iteration == 0)) & ~skipestep        
       lower = [];
      
       if(isempty(gmfits))
           lower = pdist2(data, centers, 'euclidean');% calcdist(data,centers(j,:));
       else
           lower = pdist2(data, centers, 'mahalanobis'); % 'calcdist(data,centers(j,:),gmfits{j}); 
       end
        
       [mindist, jMin] = min(lower,[],2);
       mincenter = jMin; 
...
% it's correct to have mean(data(track,:),1) but this can make answer worse!
            ...
            if method == 3
                [ centers(j,:),gmfits{j}] = calccenters(data(track,:));
            else
                centers(j,:) = mean(data(track,:),1);
            end
        end
    ...
    if method == 2
        for j = diffj
            offset = pdist2(centers(j,:),oldcenters(j,:));
...
%   [iteration toc nchanged computed size(diffj,2)]
    disp([num2str(iteration) ' ' num2str(toc) ' ' num2str(nchanged) ' ' num2str(computed)]);
...
if(method < 3)
    ...
    udist = zeros(n,1); 
    for j = 1:k
        iData = find(mincenter == j);
        udist(iData) = calcdist(data(iData,:), centers(j,:), gmfits{j}); 
    end
end
...
q2 = mean(udist.^2);
if(method ~= 3)
    disp('Euclidean KMeans final metrics:');
else
    disp('Mahal KMeans final metrics:');
end

disp('');
disp(['Iterations  Time(ms)  Quality  Quality Squared  Calculations']);
disp([num2str(iteration) ' ' num2str(toc) ' ' num2str(quality) ' ' num2str(q2) ' ' num2str(total)]);
disp('');
disp('');