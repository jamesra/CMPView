   ...
   methods
       ...
       function obj = DataCollection(Controller, Data, Names)
           if nargin > 0
               ...
               obj.Categories = ones(numDatum,1);
...
%            end
           ...
           if(~isempty(iChangedCategories))
               ...
               ChangedCategories = unique([1; OldChanged; NewChanged]);
               obj.Categories(iChangedCategories) = NewCategories(iChangedCategories);

               obj = obj.UpdateCategoryObjects();
           end
       ...
       function obj = UpdateCategoryObjects(obj)
           
            %%%%%%%Update the CategoryObjects%%%%%%%
           numCategories = unique(obj.Categories);
           
           for(iCat = numCategories')
              obj.CategoryObjects(iCat).Members = find(obj.Categories == iCat);
           end
